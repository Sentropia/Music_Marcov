...
class AudioSynthesisError(RuntimeError):
    """Raised when the server cannot render a MIDI sequence as MP3."""
...
def generate_midi_bytes(order: object, seed: int | None = None) -> bytes:
    ...
    midi = generate_midi(order, seed)
    output = BytesIO()
    midi.write(output)
    return output.getvalue()


def generate_midi(order: object, seed: int | None = None) -> pretty_midi.PrettyMIDI:
    """Generate the MIDI object used by both MIDI and browser-audio output."""
    ...
    return states_to_midi(states, TEMPO_BPM, rng)


def soundfont_path() -> Path:
    """Return the deployed SoundFont path, allowing an explicit override."""
    return Path(os.environ.get("SOUNDFONT_PATH", DEFAULT_SOUNDFONT_PATH))


def midi_to_mp3_bytes(midi: pretty_midi.PrettyMIDI) -> bytes:
    """Render MIDI to MP3 in memory using FluidSynth's Python bindings."""
    font_path = soundfont_path()
    if not font_path.is_file():
        raise AudioSynthesisError(f"SoundFont is unavailable at {font_path}.")

    try:
        import fluidsynth
        import lameenc
    except ImportError as exc:
        raise AudioSynthesisError("Audio rendering dependencies are unavailable.") from exc

    synth = None
    try:
        synth = fluidsynth.Synth(samplerate=AUDIO_SAMPLE_RATE)
        soundfont_id = synth.sfload(str(font_path))
        if soundfont_id < 0:
            raise AudioSynthesisError("FluidSynth could not load the SoundFont.")
        if synth.program_select(0, soundfont_id, 0, 0) != 0:
            raise AudioSynthesisError("FluidSynth could not select the piano preset.")

        encoder = lameenc.Encoder()
        encoder.set_bit_rate(AUDIO_BITRATE_KBPS)
        ...
        for note in notes:
            if note.start > current_time:
                pcm_parts.append(synth.get_samples(round((note.start - current_time) * AUDIO_SAMPLE_RATE)).tobytes())
            synth.noteon(0, note.pitch, note.velocity)
            pcm_parts.append(synth.get_samples(max(1, round((note.end - note.start) * AUDIO_SAMPLE_RATE))).tobytes())
            synth.noteoff(0, note.pitch)
            current_time = note.end

        # Give the final note a short tail so it is not cut off abruptly.
        pcm_parts.append(synth.get_samples(round(AUDIO_SAMPLE_RATE * 0.25)).tobytes())
        return encoder.encode(b"".join(pcm_parts)) + encoder.flush()
    except AudioSynthesisError:
        raise
    except Exception as exc:
        raise AudioSynthesisError("FluidSynth could not render the generated MIDI.") from exc
    finally:
...
def generate_mp3_bytes(order: object, seed: int | None = None) -> bytes:
    """Generate browser-compatible MP3 bytes entirely in memory."""
    return midi_to_mp3_bytes(generate_midi(order, seed))
