...
from music_generator import MAX_ORDER, MIN_ORDER, generate_mp3_bytes, load_model
...
def create_app() -> Flask:
    ...
    def healthz():
        try:
            ...
            application.logger.exception(
                "Health check could not load the Markov model"
            )
    ...
    def generate():
        ...
        try:
            mp3_data = generate_mp3_bytes(payload.get("order"))
        except ValueError as exc:
            return jsonify(error=str(exc)), 400
        except Exception:
            application.logger.exception("MP3 generation failed")
            return jsonify(
